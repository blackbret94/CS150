...
public class Schedule {
  ...
  /** Gets a day in the ArrayList
    ...
    */
  public ArrayList<Day> getDay(String dayName){
    // create ArrayList
    ArrayList<Day> matchList = new ArrayList<Day>();
    for (int i = 0; i < dayList.size(); i++){
      if (dayName.equals(dayList.get(i).getDay())){
        matchList.add(dayList.get(i));
      }
    }
  ...
  /** Checks to see if a time and day matches
    * @param time The time to check
    * @param day The day to check
    * @return True if the time matches */
  public boolean matchDayTime(String day, String time){
    // convert string to float
    Float timeFloat = stringToTime(time);
    
    // loop through list of days
    for (int i = 0; i < dayList.size(); i++){
      // check that it matches
      if (dayList.get(i).getDay().equals(day)){
        if ((dayList.get(i).getOpenTime() <= timeFloat && dayList.get(i).getCloseTime() >= timeFloat) || (dayList.get(i).getOpenTime() <= timeFloat+24.0f && dayList.get(i).getCloseTime() >= timeFloat+24.0f)){
          // return true
          return true;
        }
      }
  ...
  /** Checks to see if a day matches
    * @param day The day to check
    * @return True if the day matches */
  public boolean matchDay(String day){
    // loop through list of days
    for (int i = 0; i < dayList.size(); i++){
      // check that the listing is open at some point on that day
      if (dayList.get(i).getDay().equals(day)){
        // return true
        return true;
      }
  ...
  /** Checks to see if a time matches
    * @param time The time to check
    * @return True if the time matches */
